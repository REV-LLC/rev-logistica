...
El modelo dispone de tres herramientas de lectura: `consultar_inventario` (planes preparados para saldos actuales), `ver_esquema` (columnas de hasta cuatro vistas) y `consultar_datos` (SQL validado para otras preguntas). PostgreSQL deniega escrituras; cada consulta además verifica permisos y usa `BEGIN READ ONLY`. El parser acepta un subconjunto de SELECT y UNION sobre las vistas autorizadas, con funciones permitidas y conversiones básicas. No admite SQL arbitrario, WITH, funciones de usuario, acceso a tablas del sistema, contraseñas, tokens ni datos de configuración. Se ejecuta el SQL regenerado a partir del árbol validado.
...
No se envía la conexión de base ni su contraseña al modelo. Los logs guardan usuario, cantidad de consultas, duración y consumo de tokens/llamadas, no textos ni resultados. El historial vive en memoria de la página y se pierde al recargar; no se comparte entre usuarios. Para limitar memoria se conservan hasta seis mensajes recientes (2000 caracteres cada uno) y un presupuesto de 32000 caracteres de evidencia compacta por respuesta; el modelo recibe una marca de parcialidad al alcanzar ese presupuesto. La interfaz conserva las fuentes obtenidas completas dentro de su propio límite. Se vuelve a consultar la base al responder; el historial enviado por el navegador no se acepta como evidencia.

## Eficiencia y Excel

- Inventario actual: una herramienta busca por raíces de nombre/familia/subfamilia y agrupa antes del límite, sin explorar el catálogo ni escribir SQL primero. Conserva variantes, propietarios y saldos anómalos por separado. Fechas históricas, filtros por entidades concretas y otras estadísticas usan el esquema selectivo y SQL validado.
- El esquema completo deja de enviarse con cada pregunta. Solo los metadatos se guardan cinco minutos en memoria del servidor; las verificaciones de permisos y los saldos nunca se reutilizan entre preguntas. El endpoint de estado comprueba la conexión de nuevo.
- Las filas enviadas al modelo son listas de valores con los encabezados una sola vez. Una consulta idéntica dentro de la misma pregunta reutiliza su evidencia, sin volver a leer PostgreSQL. El contexto del modelo y el número máximo de turnos siguen acotados.
- `usage` en la respuesta incluye llamadas, tokens de entrada y salida reportados por el SDK. No equivale a un precio ni garantiza un ahorro porcentual: depende de la pregunta, los datos y los reintentos del modelo.
- El botón **Descargar Excel** genera un `.xlsx` en el navegador con un resumen y una hoja por consulta. La biblioteca se carga solo al pulsarlo. No llama a OpenAI ni vuelve a consultar la base.
- El archivo es una copia de las tablas recibidas, no una exportación ilimitada. Conserva advertencias de resultados parciales, fecha de consulta, filtros de Excel y encabezados inmovilizados. Si una fuente está recortada, pedir una agregación o una consulta más específica.
- Tablas y archivos usan encabezados y valores de categorías en español, sin traducir nombres propios. Cantidades numéricas conservan su tipo; números de más de 15 dígitos permanecen como texto para evitar redondeo de Excel. Fechas de calendario no cambian de día; marcas de tiempo se muestran en hora de Bogotá. Valores de texto no se convierten en fórmulas ni enlaces ejecutables.
- El historial y el archivo descargado son instantáneas. Para conocer el saldo actualizado hay que enviar una pregunta nueva.
