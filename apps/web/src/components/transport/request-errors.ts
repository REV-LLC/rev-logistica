...
import { ProviderRemissionRequirement } from './request-types';
export const getMissingProvidersFromApprovalError = (
  error: unknown,
  ...
  const payload =
    response.message && typeof response.message === 'object'
      ...
      : response;
  if (
    payload.code !== 'PROVIDER_REMISSION_REQUIRED' ||
...
export function formatTransportError(error: unknown, fallback: string) {
  ...
  const visibleMessage =
    !message || technicalEnglishMessage.test(message) ? fallback : message;
  return error instanceof ApiError
    ? `${error.status}: ${visibleMessage}`
    : visibleMessage;
}
