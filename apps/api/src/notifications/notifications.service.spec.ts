...
describe('NotificationsService', () => {
  ...
  it('combines document email and WhatsApp deliveries with their references', async () => {
    const document = {
      id: 'document-1',
      type: 'REMISSION',
        ...
        findMany: jest.fn().mockResolvedValue([
          {
            ...
            subject: 'Remisión RM000001 aprobada',
            attachmentNames: ['remision-RM000001.pdf', 'firma.png'],
            status: 'SENT',
            sentAt: new Date('2026-08-04T14:00:00.000Z'),
            createdAt: new Date('2026-08-04T13:59:00.000Z'),
            error: null,
            document,
          },
        ]),
    ...
    await expect(service.listCommunicationDeliveries()).resolves.toEqual([
      expect.objectContaining({
        channel: 'EMAIL',
        recipient: 'documentos@cliente.test',
        attachments: ['remision-RM000001.pdf', 'firma.png'],
        reference: expect.objectContaining({
          label: 'Remisión RM000001',
      ...
      expect.objectContaining({
        ...
        status: 'FAILED',
        attachments: ['remision-RM000001.pdf'],
      }),
    ]);
  });
  it('automatically creates the mandatory SOAT and technical inspection topics', async () => {
    ...
    expect(upsert).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        where: {
          entityType_entityId_eventType: expect.objectContaining({
      ...
      recipients: [
        {
          ...
          user: {
            ...
            employee: { name: 'Ana', lastName: 'Ruiz', phone: null },
          },
        },
      ],
    };
    ...
    expect(topicFindMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          recipients: { some: { userId: 'user-1', user: { active: true } } },
        }),
      }),
    );
    expect(reminders[0]).toMatchObject({
      topicId: 'topic-1',
      title: 'SOAT de ABC123',
      status: 'OVERDUE',
      unit: 'DAYS',
      ...
      recipients: [
        {
          ...
          user: {
            email: 'ana@example.com',
            employee: { name: 'Ana', lastName: 'Ruiz', phone: '+573001234567' },
          },
        },
      ],
    };
  ...
  it('dispatches alerts only through WhatsApp', async () => {
    ...
    const dispatchOne = jest
      .spyOn(service as any, 'dispatchOne')
    ...
    const prisma = {
      ...
      task: {
        findMany: jest
          .fn()
          .mockResolvedValue([
            { id: 'task-1', title: 'Revisar equipo', dueDate, status: 'OPEN' },
          ]),
      },
    };
    const service = new NotificationsService(
    ...
    const item = {
      ...
      plan: {
        ...
        asset: {
          ...
          publicCode: 'EQ-001',
          hourMeter: 0,
          serialOrEngine: 'SERIE-1',
          description: 'Equipo diario',
          sku: { chargeType: 'DAY' },
        },
      },
    };
